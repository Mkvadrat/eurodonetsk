...
function get_page_title($id_section_current, $id_page_current, $id_business_full_current)
{
  ...
  if ($row_m['page_title'] != '')
    ...
    $title = $row_m['title'];
  //выбор языка выдачи
  if ($pages_lng == 'pages_en')  
    $ret_str = '<title>Donetsk city portal : '.$title.'  </title>'."\n";
  else 
    $ret_str = '<title>'.$title.': Портал города Донецка </title>'."\n";
  ...
  {
	 $sql = mysql_query('SELECT * FROM  '.$catalogue_lng.'  WHERE link LIKE "%index.php?business_full='.$id_business_full_current.'%" LIMIT 1');
  ...
  {  //главная у нас или все остальное
    $ret_str = '<title>Donetsk city portal:  сайт города Донецка : русско-англоязычный портал город Донецк Украина</title>'."\n".'<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'."\n";    
  }
...
{    
    ...
    $result = mysql_query("SELECT * FROM pages WHERE page_url='$URL'")
        or die("Ошибка при выполнении запроса: ".mysql_error());
    $res = mysql_fetch_array($result);
    $id = $res['id'];
    
    if ($res=='') { // Текущей страницы нет в базе
        $result = mysql_query("INSERT INTO pages (page_url, visits) VALUES ('$URL', 1)")
            or die("Ошибка при выполнении запроса: ".mysql_error());
        $id = mysql_insert_id();
        $result = mysql_query("INSERT INTO  pages_visits (user_ip, visited_page) VALUES ('$IP', $id)")
            or die("Ошибка при выполнении запроса: ".mysql_error());
    }
    else {  // Есть в базе
        $result = mysql_query("SELECT * FROM pages_visits WHERE user_ip='$IP' AND visited_page='$id'")
            or die("Ошибка при выполнении запроса: ".mysql_error());
        $res = mysql_fetch_array($result);
        
        if ($res=='') {     // Этот пользователь не просматривал страницу
            $result = mysql_query("INSERT INTO  pages_visits (user_ip, visited_page) VALUES ('$IP', $id)")
                or die("Ошибка при выполнении запроса: ".mysql_error());
            $result = mysql_query("UPDATE pages SET visits=visits+1 WHERE id=$id")
                or die("Ошибка при выполнении запроса: ".mysql_error());
        }
    }   
    // Получение количества просмотров
    $result = mysql_query("SELECT visits FROM pages WHERE page_url='$URL'")
        or die("Ошибка при выполнении запроса: ".mysql_error());
    $res = mysql_fetch_array($result);
...
function translit($rus_str)
{
    ...
    $eng = array('a','b','v','g','d','e','jo','zh','z','i','j','k','l','m','n','o','p','r','s','t','u','f','h','ts','ch','sh','sch','','y','','e','ju','ja');
    
    $eng_str = str_replace($rus, $eng, $rus_str); 
       
    return $eng_str;
}

function links_to_cpu($straddr) 
{
    global $dbtree;
    global $lang_prefix;
    
    $url = array();
    $sections_lng = 'sections_'.$lang_prefix;
    ...
    $straddr = substr($straddr, strpos($straddr,'?')+1);
    // param1=val1&param2=val2
    $straddr = explode('&',$straddr);
    // param1=val1 param2=val2
    foreach($straddr as $val)
    {
        $strexp = explode('=',$val);
        $param = $strexp[0];
        $value = $strexp[1];
    ...
    if ((!empty($GET['section_id']))||(!empty($GET['id_page'])))
    {
        $sec_id = $GET['section_id'];
    }
    else
    if (!empty($GET['business_full']))
    {
        $business_id = $GET['business_full'];
        $sql = mysql_query("SELECT idsec1 FROM $catalogue_lng WHERE link LIKE '%index.php?business_full=$business_id%'")
    ...
    $sql = mysql_query("SELECT * FROM $sections_lng WHERE section_id = $sec_id")
        or die("Ошибка при выполнении запрос3: ".mysql_error());
    $res = mysql_fetch_array($sql);
    mysql_free_result($sql);
    ...
    $url[$i] = str_replace($replace_arr,'',mb_strtolower($res['section_name'],'utf-8'));
    if ($lang_prefix == 'ru')
        $url[$i] = translit($url[$i]);
        
    $res = $dbtree->GetParentInfo($sec_id);
    ...
    if (!empty($GET['business_full']))
    {
        $ret_str.='/'.$GET['business_full'].'/'.$GET['pm'];
    }
    ...
    if (!empty($GET['business']))
    {
        $ret_str.='/'.$GET['business'];
    }
    
    if (!empty($GET['id_page']))
    {
        $ret_str.='/'.$GET['id_page'];
    }
...
function set_cpulinks()
{
    global $lang_prefix;
    $url = '';
    $tables = array('sections_', 'catalogue_', 'pages_');    
    $section_id = $business = $rss = $business_full = $pm = $id_page = 'NULL';
    $GET = '';
    ...
    foreach ($tables as $table)
    {
        if ($table == 'pages_')
        {
            $sql = mysql_query("SELECT id_page, idsec FROM $table$lang_prefix")
                or die("Ошибка при выполнении запроса5: ".mysql_error());
        }
        
        if ($table == 'sections_')
        {
            $sql = mysql_query("SELECT section_id, section_url FROM $table$lang_prefix WHERE section_id > 1")
                or die("Ошибка при выполнении запроса4: ".mysql_error());
        }
        
        if ($table == 'catalogue_')
        {
            $sql = mysql_query("SELECT link FROM $table$lang_prefix WHERE link LIKE '%index.php?business_full=%'")
                or die("Ошибка при выполнении запроса6: ".mysql_error());
        }
        ...
        while ($row = mysql_fetch_array($sql))
        {
            if ($table == 'pages_')
            {
                $section_id = $row['idsec'];
                $id_page = $row['id_page'];
                $url = "index.php?section_id=$section_id&id_page=$id_page";
            }
            ...
            if ($table == 'sections_')
            {
                ...
                else
                {
                    ...
                    $url = "index.php?section_id=$section_id";
                }
            }
            ...
            $cpu_url = links_to_cpu($url);
            $cpu_url = str_replace(' ','',$cpu_url);
            ...
            if ($cpu_url != '')
            {
                ...
                if (!empty($GET['business_full']))
                {
                    $business_full = $GET['business_full'];
                }            
                if (!empty($GET['pm']))
                {
                    $pm = $GET['pm'];
                ...
                $sql2 = mysql_query("INSERT INTO cpulinks_$lang_prefix (cpu_url, section_id, business, rss, business_full, pm, id_page) VALUES ('$cpu_url', $section_id, $business, $rss, $business_full, $pm, $id_page)")
                    or die("Ошибка при выполнении запроса7: ".mysql_error());
...
function cpu_to_norm($cpustring)
{
    global $lang_prefix;
    global $_GET;
    
    $sql = mysql_query("SELECT section_id, business, rss, business_full, pm, id_page FROM cpulinks_$lang_prefix WHERE cpu_url LIKE '%$cpustring%'")
                or die("Ошибка при выполнении запроса8: ".mysql_error());
    ...
    if ($row = mysql_fetch_array($sql))
    {
        ...
        if ($row['section_id'] != '')
            $_GET['section_id'] = $row['section_id'];
        if ($row['business'] != '')
            $_GET['business'] = $row['business'];
        if ($row['rss'] != '')
            $_GET['rss'] = $row['rss'];
        if ($row['business_full'] != '')
            $_GET['business_full'] = $row['business_full'];
        if ($row['pm'] != '')
            $_GET['pm'] = $row['pm'];
        if ($row['id_page'] != '')
            $_GET['id_page'] = $row['id_page'];
    }
}
