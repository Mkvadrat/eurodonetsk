...
function change_language_flags($class_name, $width, $height)
{
    ...
    {                                
        if ($_GET['n_id'])
        {
            if ($new_id = get_business_association($_GET['n_id'],$lang_prefix,$row['setting_value']))
            {
            $flags_string = $flags_string.'<div class="'.$class_name.'"><a href="index.php?lang='.
                        $row['setting_value'].'&amp;n_id='.$new_id.'&amp;pm=1"><img src="images/flags/fl_'.
...
function create_main_menu($section_id)
{
    ...
    while ($row = mysql_fetch_array($sql))
    {
        ...
        if ($item = $dbtree->NextRow())
        {
            ...
            echo '</tr></tbody></table></td></tbody></td></table>';
            $gl_num++;
        }
        else
        {
...
function get_page_title($id_section_current, $id_page_current)
{
  global $pages_lng;
  global $tbl_lng;
  $ret_str = '';
  ...
  $ret_str = '<title>EuroDonetsk2012 - Евро-Донецк 2012: '.$title.'</title>'."\n";
  ...
  {
    $ret_str = '<title>EuroDonetsk2012 - Евро-Донецк 2012</title>'."\n".'<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'."\n";    
  }
...
function get_business_sections($section_id)
{
    ...
    $dbtree->Branch($section_id , array('section_id', 'section_name', 'section_level'),array('and' => array('section_level > 2')));
    while ($item = $dbtree->NextRow())
    {
...
function get_alphabet($section_id, $get_n)
{
    ...
    echo '<div class="alpha_ru">';

    for ($i=1;$i<33;$i++)
    {
        if ($alphabet_ru[$i] <>  $ch)
            echo '<div class="alpha_item"><a href="index.php?section_id=' . $section_id . '&amp;business=2&amp;n=' . strtolower($alphabet_re[$i]) . '" >' . $alphabet_ru[$i] . '</a></div>';
        else
        {
            echo '<div class="alpha_item"><a href="index.php?section_id=' . $section_id . '&amp;business=2&amp;n=' . strtolower($alphabet_re[$i]) . '" style="color: #EC5D0E;">' . $alphabet_ru[$i] . '</a></div>';
            $ch_alpha = $alphabet_ru[$i];
        }
    }
    echo '</div>';				

    echo '<div class="alpha_en">';

    for ($i=1;$i<24;$i++)
    {
        if ($alphabet_en[$i] <>  $ch)
            echo '<div class="alpha_item"><a href="index.php?section_id=' . $section_id . '&amp;business=2&amp;n=' . strtolower($alphabet_en[$i]) . '" >' . $alphabet_en[$i] . '</a></div>';
        else
        {
            echo '<div class="alpha_item"><a href="index.php?section_id=' . $section_id . '&amp;business=2&amp;n=' . strtolower($alphabet_en[$i]) . '"  style="color: #EC5D0E;">' . $alphabet_en[$i] . '</a></div>';
...
function get_enterprise($section_id)
{
    ...
    if ($level == 3)
    {
        ...
        while ($item = $dbtree->NextRow()) 
        {
            $sql = mysql_query("SELECT id FROM catalogue_".$lang_prefix." WHERE idsec like ('%".$item['section_id'].";%') ".$alpha_where)
                or die('Invalid query: '.mysql_error());
                
            while ($row = mysql_fetch_array($sql))
            {
                $mass_id[$pposs] = $row['id'];
                $pposs++;
            }
        ...
        $sql = mysql_query("SELECT id, name, logo, short_name, ind, city, street, house, email, tel, www FROM catalogue_".$lang_prefix." WHERE id in (".implode(',',$mass_id).") ".$alpha_where." ORDER BY  order_s, name")
            or die('Invalid query: '.mysql_error());
    }
    else
    {
        $sql = mysql_query("SELECT id, name, logo, short_name, ind, city, street, house, email, tel, www FROM catalogue_".$lang_prefix." WHERE idsec like ('%".$section_id.";%') ".$alpha_where." ORDER BY  order_s, name")
            or die('Invalid query: '.mysql_error());
    }
    
    while ($row = mysql_fetch_array($sql))
    {
        echo '<div class="catalogue_item">';

        if ($row['logo'])
            echo '<div><img class="bus_sec_image" src="' . $row['logo'] . '" width="60" height="60" border="0" alt=""></div>';
        else
            echo '<div><img class="bus_sec_image" src="../images/catalogue_logo.gif" width="60" height="60" border="0" alt=""></div>';
    
        echo '  <div class="catalogue_h">';
    
        if ($row['short_name'])
            echo '<a href="index.php?n_id=' . $row['id'] . '">' . stripslashes($row['name']) . '</a>';
        else
            echo $row['name'];

        echo '</div><div class="catalogue_txt"><div>';
...
function get_paid($id, $page)
{
    ...
    $sql = mysql_query('SELECT bf.page, cf.short_name FROM business_'.$lang_prefix.' bf inner join catalogue_'.$lang_prefix.' cf on cf.id = bf.business_id  where bf.page_num = '.$where.' and bf.business_id = "'.$id.'"')
        or die('Invalid query: '.mysql_error());
        
    $row = mysql_fetch_array($sql);
    
    echo '<link type="text/css" rel="stylesheet" href="root/msite/global.css" media="all">';
    echo '<link type="text/css" rel="stylesheet" href="root/msite/'.$row['short_name'].'/'.$row['short_name'].'_'.$lang_prefix.'.css" media="all">';
    echo $row['page'];
    
    mysql_free_result($sql);    
}

function get_paid_gallery($id)
{
    global $lang_prefix;
    
    $sql = mysql_query('SELECT short_name FROM catalogue_'.$lang_prefix.' where id = "'.$id.'"')
        or die('Invalid query: '.mysql_error());
    $row = mysql_fetch_array($sql);
    
    for ($i = 1; $i < 100; $i++)
    {
        if (file_exists('root/msite/'.$row['short_name'].'/gallery/'.$i.'.jpg'))
            print '<div class="paid_c_p"><a rel="lightbox-tour" href="root/msite/'.$row['short_name'].'/gallery/'.$i.'.jpg" title="Gallery">
                    <img src="root/msite/'.$row['short_name'].'/gallery/'.$i.'_s.jpg" />
                  </a></div>';
...
function get_paid_essentials($id)
{
    
    global $lang_prefix;
    
    $sql = mysql_query('SELECT name, tel, email, www, ind, city, street, house FROM catalogue_'.$lang_prefix.' where id = "'.$id.'"')
        or die('Invalid query: '.mysql_error());
    $row = mysql_fetch_array($sql);
    
    print  '<td>'.$row['ind'].' '.$row['city'].'<br> '.$row['street'].' '.$row['house'].'</td>
	    <td>'.$row['tel'].'</td>
	    <td>'.$row['email'].'</td>
            <td>'.$row['www'].'</td>';
...
function get_paid_path($id)
{
    global $lang_prefix;
    
    $sql = mysql_query('SELECT cm.idsec, sr.section_name FROM catalogue_'.$lang_prefix.' cm inner join sections_'.$lang_prefix.' sr on sr.section_id = substring(cm.idsec,1, LOCATE(\';\', cm.idsec)) WHERE cm.id = "'.$id.'"')
        or die('Invalid query: '.mysql_error());
    ...
    $idsec = substr($row['idsec'],0,strpos($row['idsec'],';'));
    
    $adress_path = get_adress($idsec);
    
    echo strtolower('<a href="index.php?section_id='.$adress_path[1].'&amp;business=1">'.$adress_path[2].'</a> / <a href="index.php?section_id='.$adress_path[3].'&amp;business=1">'.$adress_path[4].'</a> / <a href="index.php?section_id='.$adress_path[5].'&amp;business=2">'.$adress_path[6].'</a> / <a href="index.php?section_id='.$idsec.'&amp;business=2">'.$row['section_name'].'</a>');
}
...
function get_search_results($string_search, $page, $search_in_bus)
{
    ...
    if ($search_in_bus1)
    {
    $sql = mysql_query("SELECT count(cm.id) as count_rows FROM catalogue_".$lang_prefix." cm inner join sections_".$lang_prefix." sr on sr.section_id = substring(cm.idsec,1, LOCATE(';', cm.idsec)) WHERE cm.name LIKE '%".$string_search."%' or sr.section_name LIKE '%".$string_search."%'")    
    	or die("Invalid query: " . mysql_error());
    }
    else
    {
    ...
    $sql = mysql_query("SELECT name, idsec, section_name FROM catalogue_".$lang_prefix." cm inner join sections_".$lang_prefix." sr on sr.section_id = substring(cm.idsec,1, LOCATE(';', cm.idsec)) WHERE ".$ss_t." ORDER BY order_s, name")    
    	or die("Invalid query: " . mysql_error());
    }
    else
    {
    ...
    // Необходимо удалить все HTML-теги, секции javascript, пробельные символы. 
        if ($search_in_bus1)
        {
            ...
            echo '<div class="search_main"><div class="search_link">'.$i_s.".&nbsp;<a href=index.php?section_id=".substr($row['idsec'],0,strpos($row['idsec'],';'))."&amp;business=2&amp;n=".$ch.">".stripslashes($row["name"]).' ('.stripslashes($row["section_name"]).')</a></div></div>';            
        }
        else
        {
